...
class AVLTree:
    ...
    def _values_bigger_than_k(self, node: Node, k: int):
        if node is None:
            return 0

        if self.cmp(k, node.val):
            values_to_the_right = 0 if node.right is None else node.right.val_cnt + node.right.subtree_cnt
            return self._values_bigger_than_k(node.left, k) + node.val_cnt + values_to_the_right
        return self._values_bigger_than_k(node.right, k)

    def values_bigger_than_k(self, k: int):
        return self._values_bigger_than_k(self.root, k)
